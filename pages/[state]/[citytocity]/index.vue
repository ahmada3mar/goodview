...
// Fetch route data
const moving_route = ref({})
const fromCityObj = ref(null)
const fromStateObj = ref(null)
const toCityObj = ref(null)
const toStateObj = ref(null)
...
onMounted(async () => {
  // Fetch route data
  const { data } = await useFetch(`https://api.goodview-moving.com/api/routes/${city}-to-${place}`)
  ...
  fromCityObj.value = allCities.value?.find(c => c.id === moving_route.value?.moving_from_city?.id)
  fromStateObj.value = allStates.value?.find(s => String(s.id) === String(fromCityObj.value?.state_id))
  toCityObj.value = allCities.value?.find(c => c.id === moving_route.value?.moving_to_city?.id)
  toStateObj.value = allStates.value?.find(s => String(s.id) === String(toCityObj.value?.state_id))
