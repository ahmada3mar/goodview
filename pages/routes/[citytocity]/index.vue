            ...
            <div class="relative w-full md:w-[65%] items-baseline justify-center gap-6  py-10 container h-full">
              <nav
                ...
                <template v-if="fromStateObj">
                  <NuxtLink :to="`/${fromStateObj.slug}`" class="hover:underline">{{ fromStateObj.name }}
                  </NuxtLink>
                  <span>/</span>
                </template>
                <template v-if="fromCityObj">
                  <NuxtLink :to="`/${fromStateObj.slug}/${fromCityObj.slug}`" class="hover:underline">{{ fromCityObj.name }}</NuxtLink>
                  <span>/</span>
                </template>
          ...
          <div class='w-full md:w-[70%]'>
            <div class="bg-zinc-900 rounded-lg p-4 md:p-8">
              ...
              <p class="font-[300]  text-[18px]  tracking-[.3px] font-rubik leading-[1.6em]">
                ...
                move
                from {{ fromCity }} to {{ toCity }} typically falls between <strong class='text-[#ffd343]  font-bold'>${{
              ...
              <p class="font-[300]  text-[18px]  tracking-[.3px] font-rubik leading-[1.6em] my-5">Please note: These price
              ...
              <p class="font-[300]  text-[18px]  tracking-[.3px] font-rubik leading-[1.6em] my-5">At Good View Moving and
                Storage, we believe every move is unique — and so is the final cost. Several key factors can influence the
              ...
              <ul class='list-disc pl-7 font-rubik'>
                <li class="font-[300]  text-[18px]  tracking-[.3px] font-rubik leading-[1.6em] my-2"><strong
                  ...
                  can
                  increase the overall cost. For example, moving a 2-3 bedroom home will cost more than a small apartment.
                </li>
                <li class="font-[300]  text-[18px]  tracking-[.3px] font-rubik leading-[1.6em] my-2"><strong
                    class='font-[600] text-[18px]'>Distance of the Move: </strong>Some customers book long distance moving
                ...
                <li class="font-[300]  text-[18px]  tracking-[.3px] font-rubik leading-[1.6em] my-2"><strong
                    class='font-[600] text-[18px]'>Move Date and Time: </strong> Moving during peak seasons, weekends, or
              ...
              <p class="font-[300]  text-[18px]  tracking-[.3px] font-rubik leading-[1.6em] my-5"><strong
                ...
                moves
                across the country and within your area. From packing to transport, your belongings are in safe hands.</p>
              <p class="font-[300]  text-[18px]  tracking-[.3px] font-rubik leading-[1.6em] my-5"><strong
                  class='font-[600] text-[18px]'>
              ...
              <p class="font-[300]  text-[18px]  tracking-[.3px] font-rubik leading-[1.6em] my-5">Planning a move doesn't
                have
                ...
                service.</p>
              <p class="font-[300]  text-[18px]  tracking-[.3px] font-rubik leading-[1.6em] my-5">With just a few details,
                you
                can receive a fast and free quote tailored to your move. We provide honest estimates based on the size of
                your
              ...
              <p class="font-[700]  text-[18px]  tracking-[.3px] font-rubik leading-[1.6em] my-5">Get your instant quote
                today
                and see why we're considered one of the top-rated movers for both local and long-distance relocations.</p>
...
// Fetch route data
const { data: moving_route, pending: movingRoutePending, error: movingRouteError } = useFetch(`https://api.goodview-moving.com/api/routes/${city}-to-${place}`)
const { data: allCities, pending: citiesPending, error: citiesError } = useFetch('https://api.goodview-moving.com/api/city')
const { data: allStates, pending: statesPending, error: statesError } = useFetch('https://api.goodview-moving.com/api/states')

const fromCityObj = computed(() => {
  return allCities.value?.find(c => c.id === moving_route.value?.moving_from_city?.id)
})
const fromStateObj = computed(() => {
  return allStates.value?.find(s => String(s.id) === String(fromCityObj.value?.state_id))
})
const toCityObj = computed(() => {
  return allCities.value?.find(c => c.id === moving_route.value?.moving_to_city?.id)
})
const toStateObj = computed(() => {
  return allStates.value?.find(s => String(s.id) === String(toCityObj.value?.state_id))
})
...
const statePending = computed(() => movingRoutePending.value || citiesPending.value || statesPending.value)
const stateError = computed(() => movingRouteError.value || citiesError.value || statesError.value)
...
onMounted(async () => {
  ...
  // Always load map if both cities are found
  window.initMap = () => {
    if (fromCityObj.value && toCityObj.value) {
      nextTick(() => {
