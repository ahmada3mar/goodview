    ...
    <div class="bg-black text-white">
      <div class="max-w-4xl mx-auto px-6 py-8">
        ...
        <p class="font-[300] max-w-3xl font-rubik leading-[1.6rem] mb-8">
          Looking for reliable movers in North Carolina? Good View Moving and
          Storage is here to make your next move safe and stress-free.
        </p>
        <div class="mb-12">
...
const { data: allStates, error: statesError } = useFetch(
  "https://api.goodview-moving.com/api/states"
);
const { data: allRoutes, error: routesError } = useFetch(
  "https://api.goodview-moving.com/api/routes"
);
const { data: allCities, error: citiesError } = useFetch(
...
const filteredRoutes = computed(() => {
  ...
  return allRoutes.value.filter(
    (route) => String(route.moving_from) === String(currentCityObj.value.id)
  );
});
