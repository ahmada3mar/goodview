<template>
  <div v-if="blog.id" class="bg-white" id="blog-page">
    <div>
      <!-- Hero Section -->
      <div class="relative w-full h-auto md:h-[20rem] overflow-hidden" style="
        ...
        <div
          ...
          <div class="flex items-start flex-col">
            ...
            <p class="text-sm text-white font-rubik">
              Published Date: {{ blog.date }}
            </p>
          </div>
          <div class="flex items-center lg:gap-4 gap-2 mt-4">
            <p class="lg:text-[22px] text-[18px] text-white font-rubik font-normal">
              Share:
            </p>
            <div class="flex items-center gap-1 lg:gap-3">
              <ULink :to="`https://www.facebook.com/sharer/sharer.php?u=https://goodview-moving.com/blogs/${id}`"
                aria-label="share Goodview Moving on Facebook" target="_blank" class="text-white hover:underline">
                <UIcon name="i-mdi-facebook" class="w-[30px] h-[30px] text-primary-500" />
              </ULink>

              <ULink :to="`https://twitter.com/intent/tweet?url=https://goodview-moving.com/blogs/${id}&text=${encodeURIComponent(
                blog.title
              )}`" aria-label="share Goodview Moving on Twitter" target="_blank" class="text-white hover:underline">
                <UIcon name="i-mdi-twitter" class="w-[30px] h-[30px] text-primary-500" />
              </ULink>

              <ULink :to="`https://api.whatsapp.com/send?text=${encodeURIComponent(
                blog.title
              )} https://goodview-moving.com/blogs/${id}`" aria-label="share Goodview Moving on WhatsApp"
                target="_blank" class="text-white hover:underline">
                <UIcon name="i-mdi-whatsapp" class="w-[30px] h-[30px] text-primary-500" />
              </ULink>

              <ULink :to="`https://pinterest.com/pin/create/button/?url=https://goodview-moving.com/blogs/${id}&media=${blog.image
                }&description=${encodeURIComponent(blog.title)}`" target="_blank" class="text-white hover:underline"
                aria-label="share Goodview Moving on Pinterest">
                <UIcon name="i-mdi-pinterest" class="w-[30px] h-[30px] text-primary-500" />
              </ULink>

              <ULink :to="`https://reddit.com/submit?url=https://goodview-moving.com/blogs/${id}&title=${encodeURIComponent(
                blog.title
              )}`" aria-label="share Goodview Moving on Reddit" target="_blank" class="text-white hover:underline">
                <UIcon name="i-mdi-reddit" class="w-[30px] h-[30px] text-primary-500" />
              </ULink>
              <ULink :to="`https://www.linkedin.com/sharing/share-offsite/?url=https://goodview-moving.com/blogs/${id}`"
                aria-label="share Goodview Moving on LinkedIn" target="_blank" class="text-white hover:underline">
    ...
    <div class="pt-10">
      <div class="max-w-[1120px] px-6 mx-auto grid grid-cols-1 lg:grid-cols-12 gap-10">
        ...
        <div class="lg:col-span-8 flex flex-col gap-8">
          <!-- Blog Banner -->
          <img :src="blog.image" :alt="blog.image_alt" class="w-full lg:h-[400px] rounded-lg shadow-md object-cover" />
      ...
      <div class="flex justify-between mt-5 px-5 xl:px-0 mx-auto max-w-[1120px]">
        <!-- Previous Button: left -->
        <div>
          <NuxtLink v-if="prevBlog" :to="`/blogs/${prevBlog.slug}`"
        ...
        <!-- Next Button: right -->
        <div>
          <NuxtLink v-if="nextBlog" :to="`/blogs/${nextBlog.slug}`"
      ...
      <div v-if="blog.faqs?.length > 0" class="bg-primary-500 w-full p-3 md:p-10 mt-[115px] lg:mt-[152px] rounded-none">
        <div class="container max-w-[1120px] mx-auto">
          <div class="bg-black flex flex-col gap-5 justify-center -mt-[86px] lg:-mt-[151px] rounded-[10px] p-5">
...
const { data: blogs } = await useFetch(
  "https://api.goodview-moving.com/api/blogs"
);

// Current, previous, next blog nikaalo
const currentIndex = computed(() =>
  blogs.value?.findIndex((b) => b.slug == id)
);
const prevBlog = computed(() =>
  blogs.value && currentIndex.value > 0
    ? blogs.value[currentIndex.value - 1]
    : null
);
const nextBlog = computed(() =>
  blogs.value && currentIndex.value < blogs.value.length - 1
    ? blogs.value[currentIndex.value + 1]
    : null
);

// Fetch blog data
const { data: blog, error } = await useFetch(
  `https://api.goodview-moving.com/api/blogs/${id}`,
  {
    // Handle 404 errors
